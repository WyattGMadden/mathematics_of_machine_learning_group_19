...
ggtree::ggtree(train_tree, branch.length = 'none', layout = 'circular') + 
  ggtree::geom_tippoint(size=.15*as.numeric(train_BodySize$BodySize_miss),col='blue')  +
  #ggtree::geom_tippoint(size=.03*as.numeric(as.character(BodySize$BodySize_miss)),col='red') +
  ggtree::geom_tiplab()  
```
...
tries = seq(1:15)
epsilon_seq = c(0.01,1,2,4,8,16,32,64,128,512) #must be the same as line 11
#epsilon = 100
#delta_seq = c(0.001,1,2,4,8,16)

index = 0

#error <- matrix(0, length(tries)*length(delta_seq)*length(epsilon_seq), 22)
...
for(yy in tries)
{
  ...
  for(x in epsilon_seq)
  {
   ...
   stat1 = sum(train_BodySize[getPhyloGroups(train_tree)[min][[1]][[1]],]$Species %in% clade1)/length(clade1)
   stat2 = sum(train_BodySize[getPhyloGroups(train_tree)[min][[1]][[1]],]$Species %in% clade2)/length(clade2)
   ...
   error[index,21]  <-  stat1
   error[index,22]  <-  stat2
...
error %>%
  as.data.frame() %>%
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = as.numeric(as.character(V1)), x  = V16), lwd = 1) +
  facet_wrap(~V20, scales='free') +
...
error %>%
  as.data.frame() %>%
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = as.numeric((V1)), x  = V16), lwd = 1) +
  facet_wrap(~V20, scales='free') +
  ylab("SSE Testing Data") +
  xlab ("Delta") +
  facet_wrap(~V20, scales='free')
...
error %>%
  as.data.frame() %>%
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = as.numeric(V1), x  = V7, group = V20 ), lwd = 1) +
  ylab("SSE Testing Data") +
  xlab ("Lambda") +
  facet_wrap(~V20, scales='free')
...
error %>%
  as.data.frame() %>%
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = as.numeric(V21), x  = V7, group = V20 ), lwd = 2) +
  ggplot2::geom_line(aes(y = as.numeric(V22), x  = V7, group = V20 ), lwd = 2) +
  facet_wrap(~V20, scales='free') +
...
error %>%
  as.data.frame() %>%
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = as.numeric(as.character(V9)), x  = V16, group = V20 )) +
  facet_wrap(~V20, scales='free')

error %>%
  as.data.frame() %>%
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = as.numeric(V9), x  = V7, group = V20 ), lwd = 1, col = "red") +
